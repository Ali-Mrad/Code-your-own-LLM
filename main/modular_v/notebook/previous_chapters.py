...
class LayerNorm(nn.Module):
    def __init__(self,emb_dim):
        super().__init__()
        self.eps = 1e-5
        self.scale = nn.Parameter(torch.ones(emb_dim)) # traiinable neural network weights
...
def generate_text_simple(model, idx, max_new_tokens, context_size):
    for _ in range(max_new_tokens):
        ...
        with torch.no_grad(): # we don't train the model we just generate the text so we don't need to calculate the gradients
            logits = model(idx_cond)

        logits = logits[:, -1, :] # take the last row of the logits [batch_size, last_token, vocab_size]
